...
- Priorizar horas con audiencia local despierta y predispuesta: manana post-jornada, comida de viernes y prime time de noche.
- Evitar bursts: maximo 4 posts por slot normal y 2 el domingo por la noche.
...
- No confundir carga del planner con salida: la salida real pasa por `publication_schedule.json`, autoapproval/manual review, quality checks y `-Limit`.
...
| --- | --- | --- | --- | --- | --- | --- |
| Lunes | 06:30 + 14:00 | 07:30 | 09:15, 09:45, 10:15, 10:45 | 4 | `results_roundup`, `standings_roundup`, `top_scorer_update`, `race_narrative`, `milestone_story` | Recap principal del fin de semana |
| Martes | 07:00 + 14:00 | 18:30 | 20:00, 20:30, 21:00 | 3 | `ranking`, `standings_roundup` | Mantener presencia con enfoque de clasificacion |
| Miercoles | 07:00 + 14:00 | 18:30 | 20:00, 20:30, 21:00, 21:30 | 4 | `viral_story`, `metric_narrative` | Piezas conversacionales y de retencion |
| Jueves | 07:00 + 14:00 | 18:30 | 20:00, 20:30, 21:00 | 3 | `top_scorer_update`, `ranking` | Pichichi y contexto de tabla |
| Viernes | 07:00 + 14:00 | 08:00 | 13:00, 13:30, 14:00, 14:30 | 4 | `featured_match_preview`, `match_impact_scenario` | Previas antes del fin de semana |
...
- `app/config/publication_schedule.json`: define que tipos son publicables por dia y desde que hora.
- `scripts/windows/setup_scheduler.ps1`: registra las tareas y pasa `-Limit` por slot.
- `app/core/config.py`: `x_browser_stagger_seconds=1800` y `x_browser_release_action_limit=4`.
...
Si el planner muestra 37 tareas el lunes y el slot publica 4, eso es correcto: 37 es capacidad editorial, 4 es salida real en X.
