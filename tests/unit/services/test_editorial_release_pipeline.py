...
def test_editorial_approval_policy_keeps_narratives_in_manual_review(tmp_path: Path) -> None:
    session = build_session()
    try:
        seed_release_candidates(session)
...
def test_editorial_release_pipeline_keeps_narratives_manual_in_v1(tmp_path: Path) -> None:
    session = build_session()
    try:
        seed_release_candidates(session)
        add_critical_narrative_candidates(session)
