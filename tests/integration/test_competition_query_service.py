...
def test_competition_query_service_editorial_upcoming_matches_extends_scope_on_thursday() -> None:
    session = build_session()
    try:
        seed_primera_rfef_with_ud_ibiza(session)
        competition = session.scalar(select(Competition).where(Competition.code == "primera_rfef_baleares"))
        ...
        # Remove closer scheduled matches so the first upcoming reference is 8 days ahead on Thursday.
        ...
        session.add(
            Match(
                external_id="primera-thursday-preview",
                source_name="futbolme",
                source_url="https://example.com/primera/thursday-preview",
                competition_id=competition.id,
                season="2025-26",
                group_name="Grupo 2",
                round_name="Jornada 31",
                ...
                has_scorers=False,
                scraped_at=datetime(2026, 3, 31, 10, 0, tzinfo=timezone.utc),
                content_hash="primera-thursday-preview",
                extra_data=None,
            )
        )
        session.commit()
        ...
        assert len(upcoming) == 1
        assert upcoming[0].round_name == "Jornada 31"
        assert upcoming[0].match_date == date(2026, 4, 3)
        assert upcoming[0].away_team == "UD Ibiza"
    finally:
        session.close()
