...
from app.presenters.editorial_narratives import (
    render_editorial_narratives,
    render_editorial_narratives_generation,
)
from app.services.editorial_narratives import EditorialNarrativesService

app = typer.Typer(
    add_completion=False,
    help="Narrativas metricas fiables basadas en datos reales de competicion.",
...
@app.callback()
def main() -> None:
    """CLI de narrativas editoriales metricas."""
...
def show_competition(
    ...
    with session_scope() as session:
        result = EditorialNarrativesService(session).preview_for_competition(
            competition_code,
        ...
        if as_json:
            ...
            typer.echo(render_editorial_narratives(result))
...
def generate_competition(
    ...
    parsed_date = date_type.fromisoformat(reference_date) if reference_date else None
    with session_scope() as session:
        result = EditorialNarrativesService(session).generate_for_competition(
            competition_code,
            reference_date=parsed_date,
        )
        if as_json:
            ...
            typer.echo(render_editorial_narratives_generation(result))
