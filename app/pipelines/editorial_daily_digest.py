...
app = typer.Typer(
    ...
    help="Digest editorial diario para consola y envio opcional a Telegram.",
...
def _load_digest_service_class():
    last_error: Exception | None = None
    for module_name in (
        "app.services.editorial_daily_digest",
        "app.services.editorial_daily_digest_service",
    ):
        try:
            ...
            last_error = exc
            continue
        service_class = getattr(module, "EditorialDailyDigestService", None)
        if service_class is not None:
            return service_class
    if last_error is not None:
        raise RuntimeError("EditorialDailyDigestService no esta disponible en el entorno actual") from last_error
    raise RuntimeError("No se encontro EditorialDailyDigestService")
...
def _build_digest_service(settings: Settings, session):
    service_class = _load_digest_service_class()
    return _invoke_supported(service_class, session=session, settings=settings)


def _start_task_notification(
    task_name: str,
    *,
    mode: str,
    summary_metrics: dict[str, object] | None = None,
) -> tuple[TelegramEventNotifier, object]:
...
def run(
    ...
    days: int = typer.Option(1, "--days", min=1, help="Ventana de dias a incluir en el digest"),
    send_telegram: bool = typer.Option(False, "--send-telegram", help="Enviar el digest renderizado a Telegram"),
    ...
    try:
        with session_scope() as session:
            digest_service = _build_digest_service(settings, session)
            report = _invoke_supported(
                digest_service.build_report,
            ...
            console_output = digest_service.render_console(report)
            telegram_output = digest_service.render_telegram(report)
        ...
        if send_telegram:
            ...
            if dry_run:
                ...
                if not telegram_sent:
                    typer.echo("No se pudo enviar el digest a Telegram.", err=True)
            else:
                typer.echo("Telegram no configurado; digest no enviado.", err=True)
